...
public class ShippingRateController {
	...
	public String toAddPage(Model model) {
		model.addAttribute("countries", shippingRateService.listAllCountries());
	...
	@PostMapping("/add")
	public String addShippingRate(ShippingRate rate,
	...
	@GetMapping("/edit/{id}")
	public String loadShippingRate(@PathVariable("id") Integer id,
			 					   Model model,
			 					   RedirectAttributes redirectAttributes) {
		
		try {
			ShippingRate rate = shippingRateService.getById(id);
	...
	public String deleteRate(@PathVariable("id") Integer id, RedirectAttributes redirectAttributes) {
		...
		try {
			...
			redirectAttributes.addFlashAttribute("deleteMessag", "Shipping rate for "+ rate.getState()+" has been deleted successfully.");
		} catch (ShippingRateNotFoundException e) {
			redirectAttributes.addFlashAttribute("errorMessage", e.getMessage());
		}
		return listRedirectUrl;
