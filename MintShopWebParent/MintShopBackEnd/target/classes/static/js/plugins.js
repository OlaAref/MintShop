					...
					<div class="border m-2">
							...
							<div class="imgUp">
								...
								<div class="imagePreview">
									<img id="extraImage${index}" class="img-db"/>
								</div>
								<label class="btn btn-primary customize-btn"> Add Image
									<input type="file" name="extraPhoto" class="uploadFile img-db" value="Upload Photo" style="width: 0px; height: 0px; overflow: hidden;" id="extraPhoto${index}" 
										onchange="fileInfoExtra('extraPhoto${index}', 'image-size-extra${index}')" accept="image/png, image/jpeg">
								</label> 
...
function fileInfoForProduct(extraFile) {
	...
	if (/^image/.test(files[0].type)) {
		...
		reader.onloadend = function() { 
			...
			if(imageNameId.length){
				imageNameId.val(this.result.split(",")[1]);
			}
		}
	}
}
